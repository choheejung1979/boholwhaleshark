...
function renderBookingOptions(){
  ...
  if(packageSelect){
    packageSelect.innerHTML = '<option value="">Select package</option>' + cfg.packages.map(p=>`<option value="${p.slug}">${p.name}</option>`).join("");
  }
  if(pickupSelect){
...
function renderPackages(){
  ...
  root.innerHTML = cfg.packages.map((p,i)=>`
    <article class="package-card ${i===1?'featured':''}">
      ...
      <div class="package-actions"><a class="btn btn-primary" href="booking.html?package=${encodeURIComponent(p.slug)}">Book this package</a></div>
    </article>`).join("");
}
...
function setMinDate(){const d=document.querySelector('#date');if(d)d.min=new Date().toISOString().split('T')[0];}
